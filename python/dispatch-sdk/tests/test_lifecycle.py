...
from dispatch_sdk.lifecycle import install_lifecycle_hooks
...
def test_atexit_flush_uses_shutdown_timeout(monkeypatch):
    registered: List[Any] = []
    monkeypatch.setattr("dispatch_sdk.lifecycle.atexit.register", registered.append)
    monkeypatch.setattr("dispatch_sdk.lifecycle.atexit.unregister", lambda fn: None)
...
def test_init_installs_hooks_by_default_and_opt_out():
    ...
    dispatch_sdk.init(api_key="x", environment="dev", install_lifecycle_hooks=False)
    original = sys.excepthook
    try:
        dispatch_sdk.init(api_key="x", environment="production")
        assert sys.excepthook is not original
